...
public class LakeRecordReader {
   ...
   */
  public static ArrayList<IceDataEntry> getLakeRecords(String fileName) {
      ...
      try {
        while ((data = fileIn.nextLine()) != null) {
          ...
          if (fields.length == 4) {
            ...
            int daysOfIceCover = IceDataEntry.MISSING; // default value
            ...
            IceDataEntry record = new IceDataEntry(winter, freezeDate, thawDate, daysOfIceCover);
            records.add(record);
          }
        }
      }
   ...
   */
  private static ArrayList<IceDataEntry> reverse(ArrayList<IceDataEntry> records) {
    for (int i = 0; i < records.size()/2; i++) {
      IceDataEntry temp = records.get(i);
